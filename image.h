...
void imageCopy(Image *imageDestiny, Image  *imageOrigin, int x, int y){
	...
	if(_realloc == 'y')
		imageSetup(imageDestiny,  imageOrigin->width+1, imageOrigin->height+1, _realloc);

	 diferenceW = imageDestiny->width - imageOrigin->width;
	...
	//COPY
	for(i = 0; i < size; i++){
		//if(i % imageOrigin->width == 0 && i > imageOrigin->width && _realloc == 'y'){
