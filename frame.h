...
void frameCopy(Frame *frameDestiny, Frame *frameOrigin,int x, int y){
	...
	//for(int i = 0; i < frameOrigin->image.width; i++)
	//	frameOrigin->image.pixels[cursor++] = ' ';

	while(cursor < size){
//		if(frameOrigin->image.pixels[cursor] == '|');
	//		frameOrigin->image.pixels[cursor] = ' ';
//
		cursor++;
	}

	cursor = (frameOrigin->image.height -1) * frameOrigin->image.width;
...
void frameResize(Frame *frame, int newWidth, int newHeight){
	...
	frameSetup(frame, frame->title, newWidth, newHeight, 'y');
	frameClean(frame);
